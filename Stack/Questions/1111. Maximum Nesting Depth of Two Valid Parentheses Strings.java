class Solution {
    // Method to split a valid parentheses sequence into two groups with maximum depth difference
    public int[] maxDepthAfterSplit(String seq) {
        int[] ans = new int[seq.length()];  // Array to store the group assignment for each parentheses
        int countDepth = 0;  // Variable to keep track of the current depth in the parentheses sequence

        int index = 0;  // Index to iterate through the characters of the input sequence
        for (char parentheses : seq.toCharArray()) {
            if (parentheses == '(') {
                // If the character is '(', increase the depth and assign the group based on the depth parity
                countDepth += 1;
                ans[index++] = countDepth % 2;
            } else {
                // If the character is ')', assign the group based on the depth parity and decrease the depth
                ans[index++] = countDepth % 2;
                countDepth -= 1;
            }
        }

        return ans;  // Return the array representing the group assignment for each parentheses
    }
}
